...
`static注意事项`
 ...
 static的内存：一个类里面的静态成员变量和静态成员方法，它是存储在方法区的，静态成员变量是在方法区的静态域里面
 
 根据类名称访问静态成员变量时，全程和对象没有关系，只和类有关系。
 
 ！[static内存](https://github.com/ykangli/Java-Notebook/blob/master/Image/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20200914113331.png)
 
 ```java
 public class Phone {
    String brand;
    double price; //String brand, double price存储在对立面，存储的是地址0x666，指向
    static String color;
}
```
